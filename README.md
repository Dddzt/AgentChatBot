...
| 多智能体协作 | LangGraph StateGraph (Supervisor-Worker) |
...
# 创建 venv 虚拟环境（推荐，使用 pyenv 管理的 Python 3.12）
python3 -m venv venv
...
运行 Python 命令前请先确认 `venv` 已激活（`source venv/bin/activate`）。
...
```bash
pkill -f web_bot.py; sleep 2; python3 web_bot.py
```
...
│   │   └── multi_agent/            # 多智能体协作 Bot（LangGraph Supervisor-Worker）
│   │       ├── bot.py              # MultiAgentBot 对外接口
│   │       ├── graph.py            # StateGraph 构建与编译
│   │       ├── state.py            # 共享状态定义
│   │       ├── prompts.py          # Prompt 模板
│   │       └── nodes/              # Worker 节点实现
│   │           ├── supervisor.py   # Supervisor 调度 + FinalAnswer
│   │           ├── researcher.py   # 搜索专家
│   │           ├── coder.py        # 代码专家
│   │           ├── analyst.py      # 分析专家
│   │           └── writer.py       # 写作专家
...
| `multi_agent/` | `MultiAgentBot` | 基于 LangGraph Supervisor-Worker 架构，Supervisor 调度 4 个专家节点（搜索/代码/分析/写作） |
